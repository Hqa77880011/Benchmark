...
- **任务明确**：聚焦光刻 SEM 图像中的缺陷分割，不是通用图像分割示例。
- **数据闭环完整**：包含缺陷合成、数据划分、YOLO/Swin-UNet 格式导出、训练与评估。
- **评估可复现**：提供 IoU、Dice、Precision、Recall 等常用分割指标计算脚本。
- **适合扩展**：可以替换种子缺陷、背景图像和合成参数，用于新的缺陷场景。
...
1. 光刻缺陷检测/分割实验；
2. 合成数据生成与数据增强课程项目；
3. YOLO 实例分割和 Swin-UNet 语义分割对比实验；
4. 工业视觉场景下跨域泛化能力测试。
